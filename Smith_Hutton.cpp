...
// Function to evaluate internal nodes coefficients
void InternalNodesCoefficients(double rho, double dt, double gamma) {
    for (int i = 1; i < Ny + 1; i++) {
        for (int j = 1; j < Nx + 1; j++) {
            ...
            aP[i][j] = rho / dt;
            bP[i][j] = phi[i][j] * (rho / dt + DSe / vP[i][j] * (-rho * u[i][j] / 2 - gamma / dPE) +
...
// Gauss-Seidel solver over time
void GaussSeidelSolver(double maxRes, double t_count, double dt, double rho, double gamma) {
    // Time Loop
    double res = maxRes + 1;  // condition to enter the loop
    while (res > maxRes) {
        double maxDiff = 0.0;
        InternalNodesCoefficients(rho, dt, gamma);
        for (int i = 0; i < Ny+2; i++) {
            for (int j = 0; j < Nx+2; j++) {
...
int main() {
    ...
    double maxRes = 1e-4;                       // Gauss-Seidel tollerance
    ...
    // Compute Internal Coefficients
    InternalNodesCoefficients(rho, dt, gamma);

    // File to check the mesh and coefficients
    ...
    GaussSeidelSolver(maxRes, t_count, dt, rho, gamma);
